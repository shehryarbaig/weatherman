    ...
    def parse_data(self):
        ...
        for filename in weather_data_files:
            with open(os.path.join(os.getcwd(), filename), 'r') as file_data:
                # weather_headings = [heading.strip() for i, line in enumerate(file_data) if i==0 for heading in line.split(",")]
                weather_headings = []
                weather_readings = []

                for i, line in enumerate(file_data):
                    if(i==0):
                        weather_headings = [heading.strip() for heading in line.split(",")]
                    else:
                        weather_readings = [value.strip() for value in line.split(",")]
        ...
        # print(self.weather_data) 
            ...
            # lines = [line for line in file_data]
