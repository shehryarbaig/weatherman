class WeatherDataCalculator:
    ...
    def calculate_avg_month_results(self, month_weather_data):
        ...
        avg_max_temp = round(sum(record.max_temperature for record in max_temps)/len(max_temps), 1)
    
        min_temps = [record for record in month_weather_data if record.min_temperature is not None]
        avg_min_temp = round(sum(record.min_temperature for record in min_temps)/len(min_temps), 1)
        
        humidity_temps = [record for record in month_weather_data if record.mean_humidity is not None]
        avg_humidity_temp = round(sum(record.mean_humidity for record in humidity_temps)/len(humidity_temps), 1)
